...
class SelfTool(Tool):
    ...
    def description(self) -> str:
        base = (
            ...
            "- inspect (no key): full config overview — start here.\n"
            "- inspect (key): drill into a value. Dot-paths allowed "
            "(e.g. '_last_usage.prompt_tokens', 'web_config.enable').\n"
            "- modify (key, value): change config or store custom state. "
            "Custom keys persist across turns but not restarts.\n"
            "Key values: _current_iteration (current progress), "
            "max_iterations - _current_iteration = remaining iterations.\n"
            "Note: web_config and exec_config are inspectable but read-only."
        )
    ...
    def _inspect(self, key: str | None) -> str:
        ...
        if "." not in key and not _has_real_attr(self._loop, key):
            # Fallback: check _runtime_vars for keys stored by modify
    ...
    def _inspect_all(self) -> str:
        ...
        if usage:
            parts.append(f"_last_usage: {usage!r}")
