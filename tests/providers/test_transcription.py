...
@pytest.fixture()
def fw_provider(tmp_path: Path) -> FasterWhisperTranscriptionProvider:
...
def test_fw_default_script_path_is_bundled() -> None:
    p = FasterWhisperTranscriptionProvider()
    assert p.script_path.endswith("whisper/transcribe.py")


def test_fw_explicit_script_path_is_used(tmp_path: Path) -> None:
...
async def test_fw_transcribe_returns_stdout_on_success(
    fw_provider: FasterWhisperTranscriptionProvider, tmp_path: Path
) -> None:
    ...
    assert args[0] == fw_provider.uv_bin
    assert args[1] == "run"
    assert "--script" in args
    assert fw_provider.script_path in args
    assert str(audio) in args


async def test_fw_transcribe_returns_empty_on_nonzero_exit(
    fw_provider: FasterWhisperTranscriptionProvider, tmp_path: Path
) -> None:
    audio = tmp_path / "note.ogg"
    audio.write_bytes(b"fake")
...
async def test_fw_transcribe_returns_empty_on_missing_file(
    fw_provider: FasterWhisperTranscriptionProvider,
) -> None:
    result = await fw_provider.transcribe("/tmp/does_not_exist_12345.ogg")
    assert result == ""


async def test_fw_transcribe_returns_empty_on_timeout(
    fw_provider: FasterWhisperTranscriptionProvider, tmp_path: Path
) -> None:
    audio = tmp_path / "note.ogg"
    audio.write_bytes(b"fake")
...
async def test_fw_transcribe_passes_env_vars(
    fw_provider: FasterWhisperTranscriptionProvider, tmp_path: Path
) -> None:
    audio = tmp_path / "note.ogg"
    audio.write_bytes(b"fake")
    ...
    with patch(
        ...
        await fw_provider.transcribe(audio)
...
async def test_fw_transcribe_honors_custom_config(tmp_path: Path) -> None:
    audio = tmp_path / "note.ogg"
    audio.write_bytes(b"fake")
