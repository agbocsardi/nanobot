...
def _make_mock_loop(**overrides):
    ...
    # web_config mock — needed for inspect tests
...
class TestInspectPathNavigation:
    ...
    async def test_inspect_tools_returns_blocked(self):
        """tools is BLOCKED — inspect should return access error."""
        tool = _make_tool()
...
class TestModifyFree:
    ...
    async def test_modify_existing_attr_type_mismatch_rejected(self):
        ...
        tool = _make_tool()
        result = await tool.execute(action="modify", key="provider_retry_mode", value=42)
        assert "Error" in result
    ...
    async def test_modify_existing_int_attr_wrong_type_rejected(self):
        ...
        tool = _make_tool()
        result = await tool.execute(action="modify", key="max_tool_result_chars", value="big")
        assert "Error" in result
...
class TestInspectTaskStatuses:
    ...
    async def test_inspect_task_statuses_accessible(self):
        """subagents is READ_ONLY — inspect should show subagent statuses."""
        from nanobot.agent.subagent import SubagentStatus
...
# ---------------------------------------------------------------------------
# runtime vars inspect fallback (Fix #1: cross-turn memory)
# ---------------------------------------------------------------------------
...
class TestRuntimeVarsInspectFallback:
    ...
    async def test_inspect_runtime_var_after_modify(self):
        """Design doc scenario: modify then inspect should return the value."""
        tool = _make_tool()
        await tool.execute(action="modify", key="user_prefers_concise", value=True)
        result = await tool.execute(action="inspect", key="user_prefers_concise")
        assert "True" in result

    @pytest.mark.asyncio
    async def test_inspect_runtime_var_string(self):
        tool = _make_tool()
        await tool.execute(action="modify", key="current_project", value="nanobot")
        result = await tool.execute(action="inspect", key="current_project")
        assert "nanobot" in result

    @pytest.mark.asyncio
    async def test_inspect_runtime_var_dict(self):
        tool = _make_tool()
        await tool.execute(action="modify", key="task_meta", value={"step": 2, "total": 5})
        result = await tool.execute(action="inspect", key="task_meta")
    ...
    async def test_inspect_nonexistent_still_returns_not_found(self):
        tool = _make_tool()
        result = await tool.execute(action="inspect", key="never_set_key_xyz")
        assert "not found" in result
...
class TestSensitiveSubFieldBlocking:
    ...
    async def test_inspect_api_key_blocked(self):
        ...
        tool = _make_tool()
        result = await tool.execute(action="inspect", key="web_config.search.api_key")
        assert "not accessible" in result
    ...
    async def test_inspect_password_blocked(self):
        ...
        tool = _make_tool(loop)
        result = await tool.execute(action="inspect", key="some_config.password")
        assert "not accessible" in result
    ...
    async def test_inspect_secret_blocked(self):
        ...
        tool = _make_tool(loop)
        result = await tool.execute(action="inspect", key="vault.secret")
        assert "not accessible" in result
    ...
    async def test_inspect_token_blocked(self):
        ...
        tool = _make_tool(loop)
        result = await tool.execute(action="inspect", key="auth_data.token")
        assert "not accessible" in result

    @pytest.mark.asyncio
    async def test_modify_api_key_blocked(self):
        """web_config is READ_ONLY, so any modify under it is blocked."""
        tool = _make_tool()
        result = await tool.execute(action="modify", key="web_config.search.api_key", value="evil")
    ...
    async def test_modify_password_blocked(self):
        ...
        loop.some_config = MagicMock()
        tool = _make_tool(loop)
        result = await tool.execute(action="modify", key="some_config.password", value="evil")
        assert "not accessible" in result
    ...
    async def test_modify_sensitive_top_level_blocked(self):
        """Top-level key matching sensitive name must be blocked for modify."""
        tool = _make_tool()
        result = await tool.execute(action="modify", key="api_key", value="evil")
        assert "protected" in result
...
class TestSecurityAttributeProtection:
    ...
    async def test_modify_restrict_to_workspace_blocked(self):
        ...
        tool = _make_tool()
        result = await tool.execute(action="modify", key="restrict_to_workspace", value=True)
        assert "protected" in result
    ...
    async def test_modify_exec_config_blocked(self):
        ...
        tool = _make_tool()
        result = await tool.execute(action="modify", key="exec_config", value=MagicMock())
        assert "read-only" in result
    ...
    async def test_modify_web_config_blocked(self):
        ...
        tool = _make_tool()
        result = await tool.execute(action="modify", key="web_config", value=MagicMock())
        assert "read-only" in result
    ...
    async def test_modify_channels_config_blocked(self):
        ...
        tool = _make_tool()
        result = await tool.execute(action="modify", key="channels_config", value={})
        assert "protected" in result
    ...
    async def test_inspect_restrict_to_workspace_blocked(self):
        ...
        tool = _make_tool()
        result = await tool.execute(action="inspect", key="restrict_to_workspace")
        assert "not accessible" in result

    @pytest.mark.asyncio
    async def test_inspect_exec_config_allowed(self):
        """exec_config is READ_ONLY — inspect should work."""
        tool = _make_tool()
        result = await tool.execute(action="inspect", key="exec_config")
        assert "Error" not in result

    @pytest.mark.asyncio
    async def test_inspect_web_config_allowed(self):
        """web_config is READ_ONLY — inspect should work."""
        tool = _make_tool()
        result = await tool.execute(action="inspect", key="web_config")
        assert "Error" not in result
    ...
    async def test_modify_exec_config_dotpath_blocked(self):
        ...
        tool = _make_tool()
        result = await tool.execute(action="modify", key="exec_config.enable", value=False)
        assert "read-only" in result
    ...
    async def test_modify_web_config_dotpath_blocked(self):
        ...
        tool = _make_tool()
        result = await tool.execute(action="modify", key="web_config.enable", value=False)
        assert "read-only" in result
...
class TestCurrentIteration:
    ...
    async def test_inspect_current_iteration(self):
        tool = _make_tool()
        result = await tool.execute(action="inspect", key="_current_iteration")
        assert "0" in result
    ...
    async def test_modify_current_iteration_blocked(self):
        ...
        tool = _make_tool()
        result = await tool.execute(action="modify", key="_current_iteration", value=5)
        assert "read-only" in result


# ---------------------------------------------------------------------------
# _last_usage in inspect summary (Fix #5)
# ---------------------------------------------------------------------------
